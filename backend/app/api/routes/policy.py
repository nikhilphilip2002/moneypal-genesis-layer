...
def brief(req: PolicyBriefRequest, refresh: bool = False):
    if not req.regulation_ids and not req.institution_ids:
        raise HTTPException(
            400, "Select at least one regulation or institution"
        )
    cache_key = (
    ...
    try:
        result = brief_cache.cached(
            cache_key,
            lambda: policy.brief(
                req.regulation_ids, req.institution_ids, req.focus
            ),
            refresh,
        )
    except Exception:
        raise HTTPException(
            503,
            "Policy synthesis is unavailable — intelligence services not reachable",
        )
    if result is None:
        raise HTTPException(
            404, "No matching regulations or institutions found"
        )
    return result
