...
def percentile(values: Iterable[int | float], percent: int) -> float:
    ...
    if not ordered:
        return 0.0
    index = max(
        0, min(len(ordered) - 1, ceil((percent / 100) * len(ordered)) - 1)
    )
...
def usage_summary(turns: Iterable[dict[str, Any]]) -> dict[str, Any]:
    ...
    for turn in turns:
        ...
        for call in usage.get("calls") or []:
            calls.setdefault(
                str(call.get("purpose", "unspecified")), []
            ).append(int(call.get("uncached_prompt_tokens", 0)))
    ...
    return {
        "purposes": {
            purpose: {
                "count": len(values),
                "p50_uncached": percentile(values, 50),
