...
def _period_from_payload(payload: dict[str, Any]) -> str:
    plan = (
        payload.get("drilldown")
        if isinstance(payload.get("drilldown"), dict)
        else {}
    )
    ...
    if period.get("start") or period.get("end"):
        return f"{period.get('start', '')} to {period.get('end', '')}".strip(
            " to"
        )
    ...
    *,
    ...
    for card_index, result in enumerate(results):
        ...
        if result.card_type == "analysis":
            ...
            facts.extend(
                found[:max_per_result] if max_per_result is not None else found
            )
            continue
        ...
        for row_index, row in enumerate(rows):
            ...
            for field, column in numeric.items():
                ...
                if value is None:
                    continue
                if (
                    max_per_result is not None
                    and result_fact_count >= max_per_result
                ):
                    break
                facts.append(
                    Fact(
...
def _analysis_facts(result: ToolResult, card_index: int) -> list[Fact]:
    ...
    for row_index, finding in enumerate(findings):
        ...
        spec = (
            finding.get("spec")
            if isinstance(finding.get("spec"), dict)
            else {}
        )
        period_value = (
            spec.get("period") if isinstance(spec.get("period"), dict) else {}
        )
        period = str(
        ...
        output.append(
            Fact(
                ...
                label=str(
                    finding.get("label") or finding.get("step_id") or "Finding"
                ),
                value=value,
