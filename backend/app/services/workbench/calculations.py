...
def derive(
    operation: str, left: Fact, right: Fact, *, fact_id: str, label: str
) -> Fact:
    if not left.verified or not right.verified:
        raise CalculationError("derived facts require verified operands")
    if (
        operation in {"difference", "sum", "percent_change", "ratio", "share"}
    ...
    if operation == "difference":
        ...
        if right.value == 0:
            raise CalculationError("division by zero")
        value, unit, formula = (
            (left.value - right.value) / right.value * 100,
            "percent",
    ...
    *,
    ...
    if not values or len(values) != len(weights):
        raise CalculationError(
            "weighted average needs matching values and weights"
        )
        ...
        if total != 0:
            for fact in group:
                out.append(
                    _derived(
                        operation="share",
                        value=fact.value / total * 100,
                        unit="percent",
                        operands=(fact.id, out[0].id),
                        label=f"Share of total: {_describe(fact)}",
                        period=period,
                        formula="operand / total * 100",
    ...
    for position, fact in enumerate(ordered, start=1):
        ...
        out.append(
            _derived(
                ...
                unit="rank",
                operands=(fact.id,),
                label=f"Rank by {fact.label}: {_describe(fact)}",
                period=period,
                formula="1 = highest value",
                dimensions=fact.dimensions,
            )
        )
    ...
    for numerator in fields:
        for denominator in fields:
            if numerator is denominator or denominator.value == 0:
                continue
            if (
                numerator.unit in _NON_ADDITIVE_UNITS
                or denominator.unit in _NON_ADDITIVE_UNITS
            ):
                continue
            out.append(
                _derived(
...
def derived_facts(ledger: list[Fact]) -> list[Fact]:
    ...
    for fact in ledger:
        ...
        series.setdefault((card, field, fact.unit, fact.period), []).append(
            fact
        )
