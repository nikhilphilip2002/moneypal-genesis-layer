...
def infer_visual_arguments(
    ...
    *,
    ...
    if view == "kpi":
        if len(rows) != 1 or not numeric:
            raise VisualizationError(
                "kpi requires one result row with a numeric value"
            )
        return VisualizeQueryResultArguments(
            query_id=query_id,
            chart_type=view,
    ...
    if view == "scatter":
        if len(numeric) != 2:
            raise VisualizationError(
                "scatter requires exactly two numeric columns"
            )
        return VisualizeQueryResultArguments(
            query_id=query_id,
            chart_type=view,
    ...
    if view in {"grouped_bar", "stacked_area", "heatmap"}:
        ...
        if len(measures) != 1:
            raise VisualizationError(
                f"{view} requires exactly one numeric value column"
            )
        return VisualizeQueryResultArguments(
            query_id=query_id,
            chart_type=view,
    ...
    if len(fields) < 2:
        raise VisualizationError(
            f"{view} requires an x column and a numeric value"
        )
    x = fields[0]
    measures = [field for field in numeric if field != x]
    if not measures:
        raise VisualizationError(
            f"{view} requires a numeric value column after x"
        )
...
def _unit(
    columns: dict[str, dict[str, Any]], field: str, *, counted: bool = False
) -> str:
    if counted:
        return "count"
...
def _require_fields(
    ...
    missing = list(
        dict.fromkeys(field for field in requested if field not in available)
    )
    if missing:
        raise VisualizationError(
            f"unknown result field(s): {', '.join(missing)}"
        )
...
def _validate_shape(
    ...
    if not args.y:
        raise VisualizationError(
            f"{args.chart_type} requires at least one y field"
        )
    if args.chart_type == "kpi":
        ...
        if args.x is None or len(args.y) != 1:
            raise VisualizationError(
                "scatter requires one numeric x and one numeric y field"
            )
    else:
        if args.x is None:
    ...
    if args.chart_type == "donut" and (
        args.series is not None or len(args.y) != 1
    ):
        raise VisualizationError(
            "donut requires one y field and a null series"
        )
    ...
    if (
        ...
        and args.chart_type == "kpi"
        and len(rows) != 1
    ):
        raise VisualizationError(
            "kpi with aggregation none requires exactly one source row"
        )
...
def _validate_numeric(
    ...
    if invalid:
        raise VisualizationError(
            f"numeric field(s) required: {', '.join(invalid)}"
        )
...
def _aggregate(
    ...
    for key, members in grouped.items():
        ...
        for field in args.y:
            values = [
                row.get(field) for row in members if row.get(field) is not None
            ]
            if args.aggregation == "count":
                ...
                if len(numbers) != len(values):
                    raise VisualizationError(
                        f"{args.aggregation} requires numeric y field {field}"
                    )
                if not numbers:
                    item[field] = None
...
def build_visual(
    ...
    if not source_complete and args.aggregation != "none":
        raise VisualizationError(
            "cannot aggregate a truncated query result; run a bounded query"
        )
    payload = source_record.get("result_payload") or {}
    ...
    if args.chart_type == "scatter":
        ...
        x_axis = AxisSpec(
            field=label_field, label=_label(label_field), unit="text"
        )
        series_by = None
        series_specs = [
            SeriesSpec(
                field=args.x, label=_label(args.x), unit=_unit(columns, args.x)
            ),
            SeriesSpec(
                field=args.y[0],
                label=_label(args.y[0]),
    ...
    column_specs = [
        ColumnSpec(
            ...
            sensitivity=str(
                columns.get(field, {}).get("sensitivity") or "internal"
            ),
        )
