...
AGENT_SYSTEM_PROMPT = (
    ...
    "A PostgreSQL query returns raw evidence and never creates a user-facing card. After every "
    "successful database query used in the answer, call visualize_query_result and choose the "
    "presentation fields and aggregation from its stored result. Choose table when a graphical "
    "chart is not appropriate. Never submit a data-backed final answer before that presentation "
    "tool succeeds. For a follow-up asking to visualize an existing result, use the same tool. "
    "Use aggregation none when each x/series pair is already aggregated. Cite the derived "
    "current-turn query ID returned by that tool, never the historical source ID. "
    ...
    "IDs whose values directly support the narrative), visual_query_ids (successful vN "
    "visualization IDs that should render as primary visuals), and excluded_queries (objects containing "
