...
@lru_cache(maxsize=1)
def _keyword_patterns() -> list[
    tuple[str, tuple[tuple[re.Pattern, int], ...]]
]:
...
def classify_topics(text: str) -> list[str]:
    ...
    for topic_id, patterns in _keyword_patterns():
        score = sum(
            weight for pattern, weight in patterns if pattern.search(lower)
        )
        if score:
...
def topic_label(topic_id: str) -> str:
    return TOPIC_BY_ID.get(
        topic_id, Topic(topic_id, topic_id, [], [], [])
    ).label
