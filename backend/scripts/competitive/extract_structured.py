...
def is_noise(line: str) -> bool:
    ...
    if lower.startswith("http://") or lower.startswith("https://"):
        return True
    return len(lower) < 4 or any(
        pattern in lower for pattern in NOISE_PATTERNS
    )
...
def score_doc(doc: dict, category: str) -> int:
    ...
    if any(
        hint in filename for hint in CATEGORY_FILENAME_HINTS.get(category, [])
    ):
        score += 100
...
def ranked_docs(docs: list[dict], category: str, limit: int = 8) -> list[dict]:
    scored = sorted(
        ((score_doc(doc, category), doc) for doc in docs),
...
def extract_lines(
    docs: list[dict], category: str, max_lines: int = 12
) -> list[str]:
    selected = []
    seen = set()
    ...
    for doc in docs:
        for line in doc["lines"]:
            ...
            if not any(
                ...
                for keyword in CATEGORY_KEYWORDS["Products"]
            ):
                continue
...
def extract_links(
    docs: list[dict], used_docs: list[dict], max_links: int = 30
) -> list[str]:
...
def generate_swot(
    institution: Institution, extracted: dict[str, list[str]]
) -> str:
    combined = "\n".join(
        "\n".join(values) for values in extracted.values()
    ).lower()
    has_msme = any(
        term in combined
        for term in [
    ...
    has_digital = any(
        term in combined
        for term in ["upi", "mobile", "neft", "rtgs", "imps", "atm", "digital"]
    )
    has_rural = any(
        term in combined
        for term in [
    ...
    if has_msme:
        strengths.append(
            "MSME or small-business relevance appears in the extracted source material."
        )
    if has_digital:
        strengths.append(
            "Digital/payment channels improve customer access and service convenience."
        )
    if has_rural:
        strengths.append(
            "Local, branch, co-operative, or rural presence supports relationship-led distribution."
        )
    ...
    if not has_msme:
        weaknesses.append(
            "MSME positioning is not explicit in the extracted crawl text."
        )
    ...
    if institution.slug == "sidbi_msme_ecosystem_benchmark":
        opportunities.append(
            "Use this profile as a benchmark for MSME ecosystem language and product taxonomy."
        )
...
def strategic_observations(
    ...
    if extracted.get("MSME focus"):
        observations.append(
            "MSME-related positioning can be compared across institutions for product-gap analysis."
        )

    if institution.slug == "sidbi_msme_ecosystem_benchmark":
        observations.append(
            "SIDBI should be treated as the MSME ecosystem benchmark rather than a direct local-bank peer."
        )
...
def build_profile(institution: Institution, docs: list[dict]) -> str:
    grouped = {
        category: ranked_docs(docs, category) for category in CATEGORY_KEYWORDS
    }
    used_docs = []
    extracted = defaultdict(list)

    extracted["Institution profile"] = extract_lines(
        grouped["Institution profile"], "Institution profile"
    )
    extracted["Products"] = extract_products(grouped["Products"])
    extracted["MSME focus"] = extract_lines(
        grouped["MSME focus"], "MSME focus"
    )
    extracted["Public financial highlights"] = extract_lines(
        grouped["Public financial highlights"], "Public financial highlights"
    )
    extracted["Geographic presence"] = extract_lines(
        grouped["Geographic presence"], "Geographic presence"
    )
    ...
    return (
        ...
        "## Institution profile\n\n"
        + as_bullets(
            extracted["Institution profile"],
            "Not found in the crawled Markdown.",
        )
        + "\n\n## Products\n\n"
        + as_bullets(
            extracted["Products"], "Not found in the crawled Markdown."
        )
        + "\n\n## MSME focus\n\n"
        + as_bullets(
            extracted["MSME focus"],
            "No explicit MSME focus found in the crawled Markdown.",
        )
        + "\n\n## Public financial highlights\n\n"
        + as_bullets(
            extracted["Public financial highlights"],
            "Not found in the crawled Markdown.",
        )
        + "\n\n## Geographic presence\n\n"
        + as_bullets(
            extracted["Geographic presence"],
            "Not found in the crawled Markdown.",
        )
        + "\n\n## AI-generated SWOT\n\n"
        ...
        + "\n".join(
            f"- {item}"
            for item in strategic_observations(institution, extracted)
        )
        + "\n\n## Links to source documents\n\n"
        + (
            "\n".join(links)
            if links
            else "- No source URLs found in raw Markdown."
        )
        + "\n"
    )
...
def extract_institution(
    institution: Institution, raw_root: Path, structured_root: Path
) -> Path:
...
def run(args: argparse.Namespace) -> None:
    ...
    for institution in institutions:
        output_path = extract_institution(
            institution, raw_root, structured_root
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Rule-based structured extraction from institution raw Markdown."
    )
    default_config = (
        DEFAULT_CONFIG_DIR
    ...
    parser.add_argument(
        ...
        help="Folder containing per-institution raw Markdown.",
    )
    parser.add_argument(
        "--structured-root",
        default=str(DEFAULT_STRUCTURED_ROOT),
        help="Folder for structured outputs.",
    )
    parser.add_argument(
        "--only",
        nargs="*",
        default=[],
        help="Optional institution slug(s) to extract.",
    )
