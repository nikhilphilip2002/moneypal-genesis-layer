...
def test_registry_round_trip_is_sorted(tmp_path):
    ...
    assert json_registry.load_one(tmp_path, "alpha") == {"id": "alpha", "value": 1}
    assert [item["id"] for item in json_registry.load_all(tmp_path)] == ["alpha", "zeta"]
...
@pytest.mark.parametrize("item_id", ["", "../outside", "nested/file", ".hidden"])
