...
def test_query_ids_distinguish_retry_attempts_from_corrected_sql(monkeypatch):
    ...
    original = NativeToolCall(
        id="call-1", name="query", arguments={"sql": "SELECT 1"}
    )
    first = agent._register_database_queries(state, [original])["call-1"]
    first["status"] = "timeout"

    retry = NativeToolCall(
        id="call-2", name="query", arguments={"sql": "SELECT 1"}
    )
    second = agent._register_database_queries(state, [retry])["call-2"]
    second["status"] = "success"
    corrected = NativeToolCall(
        id="call-3", name="query", arguments={"sql": "SELECT 2"}
    )
...
async def test_tool_trace_streams_running_then_completed_with_arguments(
    ...
    assert {payload["query_id"] for _event, payload in lifecycle} == {
        "turn-1:q1"
    }
...
async def test_cancelled_database_query_is_finalized_in_registry(monkeypatch):
    call = NativeToolCall(
        id="call-cancel", name="query", arguments={"sql": "SELECT 1"}
    )
    ...
    while not state["emit"].empty():
        frames.append(_sse_payload(state["emit"].get_nowait()))
    failed = next(
        payload for event, payload in frames if event == "query_failed"
    )
...
async def test_cancellation_finalizes_all_running_trace_kinds_once():
    ...
    terminal = [
        ...
        if step.get("detail") == "Stopped by user"
    ]
    assert {step["id"] for step in terminal} == {
        "model-1",
        "tool-1",
        "status-1",
    }
    ...
    assert (
        next(step for step in terminal if step["id"] == "tool-1")["call_id"]
        == "call-1"
    )
...
async def test_cancellation_finalizes_all_unfinished_queries_once():
    ...
    assert all(
        payload["error_code"] == "CANCELLED" for _event, payload in events
    )
    ...
    trace = [
        {
            ...
            "kind": "model",
            "status": "complete",
            "label": "Model deciding next action",
            "detail": "Selected 1 tool call(s)",
            "elapsed_ms": 12,
            "duration_ms": 10,
        }
    ]
...
def test_saved_conversation_returns_query_registry(monkeypatch):
    ...
    registry = [
        {
            "query_id": f"{turn_id}:q1",
            "attempt_id": f"{turn_id}:q1:a1",
