...
def test_source_groups_match_the_product_contract():
    ...
    assert (
        access.source_group("knowledge") is access.SourceGroup.LOCAL_KNOWLEDGE
    )
    assert access.source_group("macro") is access.SourceGroup.EXTERNAL_INDEXED
    assert (
        access.source_group("competitive")
        is access.SourceGroup.EXTERNAL_INDEXED
    )
    assert (
        access.source_group("regulatory")
        is access.SourceGroup.EXTERNAL_INDEXED
    )
...
def test_consent_cannot_grant_role_or_deployment_capability(monkeypatch):
    director = access.build_policy(
        role="gicc_director", external_sources_enabled=True
    )
    ...
    monkeypatch.setattr(
        access.settings, "workbench_external_connectors_enabled", False
    )
    killed = access.build_policy(role="admin", external_sources_enabled=True)
    assert not (
        {"macro", "competitive", "regulatory", "web"}
        & set(killed.effective_sources)
    )
...
def test_history_persists_latest_consent_and_turn_snapshot(monkeypatch):
    ...
    assert turn["source_policy"]["effective_sources"] == list(
