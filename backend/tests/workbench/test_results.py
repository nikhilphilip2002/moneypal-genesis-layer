...
from app.services.workbench.results import (
    Evidence,
    MAX_EVIDENCE_ITEMS,
    MAX_EXCERPT_CHARS,
    ToolResult,
...
def test_typed_result_separates_payload_evidence_and_lineage():
    result = ToolResult(
        source="db",
        card_type="chart",
        payload={"rows": [{"secret": "ui-only"}]},
        summary="Outstanding is 10.",
        evidence=[Evidence("Outstanding is 10.", untrusted=False)],
        sensitive=True,
        lineage={"sql": "SELECT hidden"},
    )
...
def test_evidence_is_bounded_and_serializable():
    result = ToolResult(
        ...
        payload={},
        evidence=[
            Evidence("x" * (MAX_EXCERPT_CHARS + 10))
            for _ in range(MAX_EVIDENCE_ITEMS + 2)
        ],
    )
...
def test_numeric_grounding_guard_accepts_format_variants_and_rejects_new_figures():
    ...
    assert composer.numbers_are_grounded(
        "PAR was 4.2% on a 1200.50 portfolio.", evidence
    )
...
def test_extractive_fallback_uses_evidence_not_ui_payload():
    result = ToolResult(
        source="web",
        card_type="brief",
        payload={"raw": "do not expose"},
        evidence=[
            Evidence("Official result", document="RBI", url="https://rbi.org")
        ],
    )
