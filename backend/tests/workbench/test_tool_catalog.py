...
async def test_catalog_filters_policy_without_mutating_canonical_definitions():
    ...
    assert curated["function"]["parameters"]["properties"]["domain"]["enum"] == [
        "concepts"
    ]
...
@pytest.mark.anyio
async def test_catalog_fails_when_discovered_local_tool_has_no_runtime_policy(monkeypatch):
    ...
    with pytest.raises(ToolCatalogError, match="missing runtime policy: unclassified"):
        await catalog.discover_local()
