...
def test_reconciliation_selects_one_local_query_and_derives_exclusions():
    ...
    result = reconcile_query_attribution(
        registry,
        FinalSynthesis(insights="Answer", query_id=2, view="bar"),
    )
...
def test_reconciliation_rejects_unknown_or_unsuccessful_selection():
    ...
    failed = reconcile_query_attribution(
        registry,
        FinalSynthesis(insights="", query_id=1, view="table"),
    )
    missing = reconcile_query_attribution(
        registry,
        FinalSynthesis(insights="", query_id=2, view="table"),
    )
...
def test_reconciliation_attributes_prior_query_without_excluding_old_executions():
    ...
    assert result.invalid_query_ids == []
    assert [item.query_id for item in result.excluded_queries] == [
        "current:q3"
    ]
